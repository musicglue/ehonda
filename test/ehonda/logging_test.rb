...
describe Ehonda::Logging do
  ...
  it 'formats simple strings' do
    ...
    @output.string.gsub(/\e\[(\d+)m/, '').must_match /severity="info " message="some string"/
  end

  it 'formats hashes' do
    @logger.warn foo: 1, bar: true, zoo: :wat?
    @output.string.gsub(/\e\[(\d+)m/, '').must_match /severity="warn " foo="1" bar="true" zoo="wat\?"/
  end
  ...
  it 'formats hashes containing errors' do
    begin
      ...
      @output.string.gsub(/\e\[(\d+)m/, '').must_match /severity="error" foo="1" error="whoops" backtrace=".+"/
    end
  end
end
